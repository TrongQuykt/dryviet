...
export const metadata: Metadata = {
  title: 'Kiến Thức Ngành | Trung Tâm Dữ Liệu DryViet',
    ...
    <div className="bg-[#FAFBFC]">
      ...
      <section className="relative pt-24 md:pt-32 pb-12 md:pb-20 overflow-hidden border-b border-slate-100">
        <div className="absolute inset-0 z-0 overflow-hidden">
          <div className="absolute inset-0 bg-[url('/images/banner/banner.jpg')] bg-cover bg-center opacity-5 blur-sm transform scale-110" />
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-white/40 to-white" />
        </div>

        <div className="container-xl relative z-10 text-center px-4">
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-brand-50 text-brand-700 rounded-full text-[8.5px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] mb-6 md:mb-8">
            <span className="w-1.5 h-1.5 rounded-full bg-brand-500 animate-pulse" />
            Vạn Cường Thịnh Insight
          </div>
          <h1 className="text-3xl md:text-5xl lg:text-7xl font-bold mb-4 md:mb-8 font-display tracking-tight text-brand-950 leading-[1.1]">
            Góc Nhìn <br className="hidden lg:block" />
            <span className="text-brand-600 mt-1 inline-block">Chuyên Ngành.</span>
          </h1>
          <p className="max-w-xl mx-auto text-slate-500 text-sm md:text-lg font-medium leading-relaxed mb-8 md:mb-10">
              ...
              return (
                <article key={post.slug} className="group flex flex-col h-full bg-[#FDFBF9] rounded-2xl md:rounded-[2.5rem] overflow-hidden border border-brand-50 transition-all duration-700 hover:shadow-2xl hover:shadow-brand-900/10">
                  <div className="relative aspect-square md:aspect-[16/10] overflow-hidden bg-slate-100">
                    ...
                    <div className={`absolute top-2 left-2 md:top-6 md:left-6 px-2 md:px-3 py-0.5 md:py-1 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest border backdrop-blur-md ${catStyle.split(' ')[0]}/80 ${catStyle.split(' ').slice(1).join(' ')}`}>
                      {post.category}
                    </div>
                  </div>
                  ...
                  <div className="p-4 md:p-10 flex flex-col flex-grow">
                    ...
                    <h3 className="text-xs md:text-xl font-bold text-slate-900 mb-2 md:mb-4 group-hover:text-brand-800 transition-colors line-clamp-2 italic tracking-tight leading-tight">
                      <Link href={`/blog/${post.slug}`}>
                        {post.title}
                      </Link>
                    </h3>

                    <p className="hidden md:block text-slate-500 text-sm leading-relaxed mb-8 flex-grow line-clamp-3 font-medium">
                      {post.excerpt}
                    </p>
                    ...
                    <div className="mt-auto pt-4 md:pt-6 border-t border-brand-50 relative z-10">
                      <Link
                        href={`/blog/${post.slug}`}
                        className="inline-flex items-center gap-2 md:gap-3 text-[8px] md:text-[10px] font-black uppercase tracking-widest md:tracking-[0.3em] text-brand-950 hover:text-brand-600 transition-all group/link"
                      >
                        Đọc Ngay
